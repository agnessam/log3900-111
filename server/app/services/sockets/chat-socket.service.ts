...
import { CHAT_NAMESPACE_NAME, CONNECTION_EVENT_NAME, TEXT_MESSAGE_EVENT_NAME } from '@app/services/socket-constants';
...
export class ChatSocketService extends SocketServiceInterface{
    ...
    protected setupSocketOns(){
        this.namespace.on(CONNECTION_EVENT_NAME, (socket: Socket) => {
            console.log("Chat socket user has connected");
            super.setOnRoom(socket);
