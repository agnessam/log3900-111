...
export class FeatherCommand implements ICommand {
  ...
  constructor(
    ...
    @Inject("featherAttributes") private featherAttributes: Feather
  ) {}
  ...
  execute(): void {
    if (this.feather) {
      ...
      this.renderer.setStyle(
        this.feather,
        "stroke",
        this.featherAttributes.stroke
      );
      this.renderer.setStyle(
        this.feather,
        "opacity",
        this.featherAttributes.strokeOpacity
      );
      this.drawingService.addObject(this.feather);
    }
  }
  ...
  updateCurrentPath(borderPointList: Point[]): void {
    ...
    if (!this.currentPath) {
      this.currentPath = this.renderer.createElement(
        "path",
        "svg"
      ) as SVGPathElement;
