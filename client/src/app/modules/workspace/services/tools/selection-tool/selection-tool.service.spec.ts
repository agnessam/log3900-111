...
describe("SelectionToolService", () => {
  ...
  const svgContour = document.createElement(
    "polygon"
  ) as Element as SVGPolygonElement;
  const svgInversement = document.createElement(
    "polygon"
  ) as Element as SVGElement;
  const svgCtrl = document.createElement("g") as Element as SVGElement;
  let spyDrawingService: any;

  beforeEach(() => {
    const spyOffset = jasmine.createSpyObj("OffsetManagerService", [
      "offsetFromMouseEvent",
    ]);
    const spyGrid = jasmine.createSpyObj("GridService", ["activateMagnetism"]);
    const spyMagnetism = jasmine.createSpyObj("MagnetismService", [
      "movementMagnetism",
    ]);
    const spySelectionTransform = jasmine.createSpyObj(
    ...
    rendererSpy.createElement
      .withArgs("polygon", "svg")
      .and.returnValue(svgContour);
    rendererSpy.createElement
      .withArgs("rect", "svg")
      .and.returnValue(svgInversement);
    rendererSpy.createElement.withArgs("g", "svg").and.returnValue(svgCtrl);

    spyDrawingService = jasmine.createSpyObj("DrawingService", [
      "addObject",
      "removeObject",
      "getObject",
      "getObjectList",
  ...
  it("should be created", () => {
    ...
    expect(
      TestBed.get(RendererProviderService).renderer.createElement
  ...
  it("#onMove should do nothing", () => {
    const service: SelectionToolService = TestBed.get(SelectionToolService);
    offsetManagerServiceSpy.offsetFromMouseEvent.and.returnValue({
      x: 10,
      y: 12,
    });
  ...
  it("#onMove should do nothing because theres was a rotation", () => {
    ...
    const mouseEvent = new MouseEvent("mousedown", { buttons: 1 });
    selectionTransfomServiceSpy.getCommandType.calls.reset();
    selectionTransfomServiceSpy.getCommandType.and.returnValue(
  ...
  it("#onPress mouse should be inside selection", () => {
    ...
    spyOn(svgContour, "getBoundingClientRect").and.returnValue(
      new DOMRect(0, 0, 40, 50)
    );
    ...
    expect(spy).not.toHaveBeenCalled();
    expect(
      TestBed.get(RendererProviderService).renderer.appendChild
  ...
  it("#onPress right click mouse should set the inverse selection", () => {
    ...
    drawingServiceSpy.getObjectList.and.returnValue(
      new Map<number, SVGElement>()
    );
  ...
  it("#setSelection should set selection around 1 object", () => {
    ...
    expect(
      TestBed.get(RendererProviderService).renderer.setAttribute
  ...
  it("#setSelection should set selection around 4 object", () => {
    ...
    spyOn(svgRect1, "getBoundingClientRect").and.callFake(
      () => new DOMRect(0, 5, 5, 5)
    );
    ...
    spyOn(svgRect2, "getBoundingClientRect").and.returnValue(
      new DOMRect(5, 0, 5, 5)
    );
    ...
    spyOn(svgRect3, "getBoundingClientRect").and.returnValue(
      new DOMRect(5, 10, 5, 5)
    );

    const svgRect4 = document.createElement(
      "rect"
    ) as Element as SVGRectElement;
    svgRect4.setAttribute("id", "4");
    svgRect4.style.stroke = "black";
    svgRect4.style.strokeWidth = "8";
    spyOn(svgRect4, "getBoundingClientRect").and.returnValue(
      new DOMRect(10, 5, 5, 5)
    );
    ...
    spyOn(svgRect5, "getBoundingClientRect").and.returnValue(
      new DOMRect(0, 5, 5, 5)
    );
    ...
    expect(
      ...
      svgContour,
      "points",
      "0,-2 7.5,-2 15,-2 15,8 15,18 7.5,18 0,18 0,8"
    );
  });
  ...
  it("#setSelection should set selection with a marker", () => {
    ...
    expect(
      ...
      svgContour,
      "points",
      "-45,-50 7.5,-50 60,-50 60,2.5 60,55 7.5,55 -45,55 -45,2.5"
    );
  });
  ...
  it("#findObjects should fond the object inside de selection rectangle and select it", () => {
    ...
    spyOn(svgRect, "getBoundingClientRect").and.callFake(
      () => new DOMRect(0, 5, 5, 5)
    );
  ...
  it("#findObjects should find the objects inside de invertion rectangle and select them", () => {
    ...
    spyOn(svgInversement, "getBoundingClientRect").and.returnValue(
      new DOMRect(0, 0, 100, 100)
    );
  ...
  it("#setSize set size of object on mouse move", () => {
    ...
    expect(
      ...
      svgContour,
      "points",
      "50.5,60.5 55,60.5 59.5,60.5 59.5,70 59.5,79.5 55,79.5 50.5,79.5 50.5,70"
    );
  });
  ...
  it("#setSize set size of object to 0 on mouse move with negative height and width", () => {
    ...
    expect(
      ...
      svgContour,
      "points",
      "50.5,60.5 50.5,60.5 50.5,60.5 50.5,60.5 50.5,60.5 50.5,60.5 50.5,60.5 50.5,60.5"
    );
  });
  ...
  it("#setSize set size of object to positive value on mouse move with negative height and width", () => {
    ...
    expect(
      ...
      svgContour,
      "points",
      "40.5,40.5 45,40.5 49.5,40.5 49.5,50 49.5,59.5 45,59.5 40.5,59.5 40.5,50"
    );
  });
  ...
  it("#getRectSelectionOffset should return first point", () => {
    ...
    service = {
      ...TestBed.get(SelectionToolService),
      getRectSelectionOffset:
        TestBed.get(SelectionToolService).getRectSelectionOffset,
      pointsList: [
        { x: 2, y: 3 },
        { x: 4, y: 5 },
        { x: 6, y: 7 },
  ...
  it("#rotationAction should create a command, rotate and set size due to shift pressed", () => {
    ...
    const svgRect = document.createElement("rect") as Element as SVGElement;
    const spy = spyOn(svgRect, "getBoundingClientRect").and.returnValue(
      new DOMRect(0, 0, 0, 0)
    );
    spyOn(svgContour, "getBoundingClientRect").and.returnValue(
      new DOMRect(1, 1, 40, 50)
    );
  ...
  it("#onKeyUp should call resizeWithLastOffset", () => {
    ...
    expect(
      selectionTransfomServiceSpy.resizeWithLastOffset
  ...
  it("#hideSelection should set visibility to hidden", () => {
    ...
    expect(rendererServiceSpy.renderer.setAttribute).toHaveBeenCalledWith(
      service["ctrlG"],
      "visibility",
      "hidden"
    );
