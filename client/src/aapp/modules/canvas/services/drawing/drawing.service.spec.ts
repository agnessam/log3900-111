...
import { RendererProviderService } from "../../../workspace/services/renderer-provider/renderer-provider.service";
...
describe("DrawingService", () => {
  ...
  it("#newDrawing should reset the drawing for a new drawing", () => {
    const spy = rendererSpy.createElement.and.returnValue(
      document.createElementNS("svg", "svg")
    );
    let called = false;
    service.drawingEmit.subscribe(() => {
      called = true;
    });
  ...
  it("#openDrawing should open a new drawing", () => {
    ...
    const svg: SVGElement = rendererProviderService.renderer.createElement(
      "svg",
      "svg"
    );
    rendererProviderService.renderer.setAttribute(svg, "width", "100");
    rendererProviderService.renderer.setAttribute(svg, "height", "100");
    rendererProviderService.renderer.setStyle(
      svg,
      "backgroundColor",
      "rgba(150,200,120,0.8)"
    );
    rendererProviderService.renderer.appendChild(
      svg,
      rendererProviderService.renderer.createElement("rect", "svg")
    );
