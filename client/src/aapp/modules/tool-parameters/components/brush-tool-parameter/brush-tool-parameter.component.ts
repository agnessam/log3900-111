...
import { TexturesService } from "src/aapp/modules/workspace/services/textures/textures.service";
...
export class BrushToolParameterComponent implements OnInit, AfterViewInit {
  ...
  @ViewChild("textureSvg") textureSvg: ElementRef<SVGElement>;
  ...
  ngAfterViewInit(): void {
    for (const texture of this.textureList) {
      this.rendererProviderService.renderer.appendChild(
        this.textureSvg.nativeElement,
        texture
      );
    }
  }
  ...
  get selectedTextureId() {
    return (
      "url(#" +
      (this.textureList[this.selectedTexture].firstChild as any).id +
      ")"
    );
  }
