...
export class BrushCommand implements ICommand {
  ...
  getBrush(): SVGPolylineElement | null {
    return this.brush;
  }
  ...
  execute(): void {
    ...
    if (this.brush) {
      ...
      const texturePattern: SVGPatternElement = this.texture.children.item(
        0
      ...
      if (this.brushAttributes.pointsList.length <= 1) {
        ...
        this.renderer.setAttribute(
          this.dot,
          "cx",
          this.brushAttributes.pointsList[0].x.toString() + "px"
        );
        this.renderer.setAttribute(
          this.dot,
          "cy",
          this.brushAttributes.pointsList[0].y.toString() + "px"
        );
        this.renderer.setAttribute(
          this.dot,
          "r",
          (this.brushAttributes.strokeWidth / 2).toString() + "px"
        );
        ...
        this.brush = this.renderer.createElement(
          "polyline",
          "svg"
        ...
        this.renderer.setStyle(
          this.brush,
          "stroke-width",
          this.brushAttributes.strokeWidth.toString() + "px"
        );
        this.renderer.setAttribute(this.brush, "points", this.pointString());
        ...
        this.renderer.setAttribute(
          this.brush,
          "fill",
          this.brushAttributes.fill
        );
        this.renderer.setAttribute(this.brush, "stroke", `url(#${textureId})`);
        this.renderer.setStyle(
          this.brush,
          "fillOpacity",
          this.brushAttributes.fillOpacity
        );
        this.renderer.setStyle(
          this.brush,
          "strokeOpacity",
          this.brushAttributes.strokeOpacity
        );
        this.drawingService.addObject(this.brush);
      }
    }
